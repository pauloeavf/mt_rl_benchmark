...
parser.add_argument("-test_turk", required=False,
                     help="Path to the test combined turk data")
...
def makeData(which, srcFile, tgtFile, srcDicts, tgtDicts, turkFile=''):
    src, tgt, turk = [], [], []
    sizes = []
    count, ignored = 0, 0
    ...
    if turkFile != '':
        tF = open(turkFile, encoding='utf-8')

    while True:
        srcWords = srcF.readline().split()
        tgtWords = tgtF.readline().split()
        if turkFile != '':
            tWords = [s.split() for s in tF.readline().split('<sep>')]
    ...
    return src, tgt, range(len(src)), turk


def makeDataGeneral(which, src_path, tgt_path, dicts, turkFile=''):
    print("Preparing " + which + "...")
    res = {}
    res["src"], res["tgt"], res["pos"], res['turk'] = makeData(which, src_path, tgt_path,
        dicts["src"], dicts["tgt"], turkFile)
    return res
